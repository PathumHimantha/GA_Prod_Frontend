...
import { getCenters, getDailyCenters } from "@/lib/centerHelper";
...
const DailyCashierPayments = () => {
  ...
  */
  ...
  /*
  ---------------------------
  LOAD CENTERS WHEN BRANCH CHANGES (DAILY ONLY)
  ---------------------------
  */
  ...
  useEffect(() => {
    ...
    const loadCenters = async () => {
      ...
      const centers = await getDailyCenters(
        formData.branch,
        dayName,
        user?.id?.toString() || "",
        "daily", // ← only daily loan centers (ccode >= 800)
      );
  ...
  useEffect(() => {
    ...
    const loadCustomers = async () => {
      ...
      const formatted = customers.map((c: any) => {
        const status = c.due_date < todayDate ? "LAP" : "ACTIVE";
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="w-full px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">
              Daily Cashier Payments
            </h1>
            <p className="text-sm text-gray-500">
              Manage and process daily loan customer payments
            </p>
          </div>
        ...
        <FloatWarningBanner
          ...
          title="Daily Cashier Payments Disabled"
...
export default DailyCashierPayments;
